...
module ProxyControllersMethods
  ...
  def index(controller_type = nil)
    ...
    if %w[contact user].include? controller_type
      ...
      refresh_token = cookies["mints_#{controller_type}_refresh_token".to_sym]
    ...
    if url_need_cache
      ...
      send_mints_request(full_url, headers, controller_type, true, time: time)
    else
      send_mints_request(full_url, headers, controller_type)
    end
  end
  ...
  def send_mints_request(full_url, headers, controller_type, cache_result = false, time: 30)
    ...
    reverse_proxy @host, headers: headers, verify_ssl: false do |config|
      ...
      if %w[contact user].include? controller_type
        config.on_response do |_status_code, response|
          ...
          cookies["mints_#{controller_type}_session_token".to_sym] = { value: response.header['Access-Token'], secure: true, httponly: true, expires: Time.at(session_token_expires_at) } if response.header['Access-Token']
          cookies["mints_#{controller_type}_refresh_token".to_sym] = { value: response.header['Refresh-Token'], secure: true, httponly: true, expires: Time.at(refresh_token_expires_at) } if response.header['Refresh-Token']
        end
      end
